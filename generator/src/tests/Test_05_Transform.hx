...
class Test_05_Transform {
	...
	function path(original:String)
		return { original:original, computed:original };  // FIXME
	...
	public function test_003_element_counting()
	{
		...
		Assert.same(
			expand(DElemList([
				...
				@id("c")@wrap(8,1)DFigure(1,MarginWidth,path("f"),@skip(2+1)@len(1)Word("c"),@skip(2)@len(2)Word("cp"))])),  // 3-1 -> Chapter3,Fig1
